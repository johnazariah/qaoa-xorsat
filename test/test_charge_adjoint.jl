...
@testset "Charge adjoint differentiation" begin
    ...
    @testset "disk spill toggle parity" begin
        params = TreeParams(2, 3, 3)
        angles = QAOAAngles([0.2, 0.35, 0.5], [0.45, 0.25, 0.15])

        val_mem, γ_mem, β_mem = charge_expectation_and_gradient(params, angles; clause_sign=-1)

        mktempdir() do dir
            val_spill, γ_spill, β_spill = charge_expectation_and_gradient(
                params,
                angles;
                clause_sign=-1,
                cache_disk_dir=dir,
            ...
            @test val_spill ≈ val_mem atol = 1e-12
            @test γ_spill ≈ γ_mem atol = 1e-9
            @test β_spill ≈ β_mem atol = 1e-9
