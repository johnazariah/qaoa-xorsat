...
@testset "Optimization" begin
    ...
    @testset "optimize_angles checkpointed Double64" begin
        mktempdir() do dir
            ...
            @test depth(result.angles) == 1
            @test isfile(joinpath(dir, "angle-snapshots.csv"))
            @test isfile(joinpath(dir, "start-1-latest-angle-snapshot.csv"))
        end
    end

    @testset "optimize_angles charge spill toggle" begin
        mktempdir() do dir
            result = optimize_angles(
                TreeParams(2, 3, 1);
                clause_sign=-1,
    ...
    @testset "durable optimizer snapshots resume starts" begin
        mktempdir() do dir
            snapshot = AngleSnapshot(
                2, 3, 1, 1, 3, 1, 5.0, 0.65, 1.0e-3, 1.0e-4, 2.0e-3,
                QAOAAngles([0.7], [0.3]), :best_seen, "2026-06-05T00:00:00Z",
            )
            write_latest_angle_snapshot!(joinpath(dir, "start-1-latest-angle-snapshot.csv"), snapshot)

            best = read_best_angle_snapshot(joinpath(dir, "start-1-latest-angle-snapshot.csv"), 2, 3, 1, 1)
            @test best !== nothing
            @test best.angles.γ == [0.7]
            @test best.angles.β == [0.3]
            ...
            result = optimize_angles(
                ...
                restarts=0,
                maxiters=1,
                initial_guesses=[QAOAAngles([0.0], [0.0])],
                rng=MersenneTwister(37),
                angle_snapshot_dir=dir,
                persist_angle_snapshots=false,
                on_angle_snapshot=s -> push!(snapshots, s),
            )

            @test QaoaXorsat.is_valid_qaoa_value(result.value)
            @test !isempty(snapshots)
            @test snapshots[1].angles.γ[1] ≈ 0.7 atol = 1.0e-12
            @test snapshots[1].angles.β[1] ≈ 0.3 atol = 1.0e-12
        end
    end
    ...
    @testset "optimization policy constructors" begin
        ...
        @test !low_mem.charge_cache_spill

        spill_mem = QaoaXorsat.optimization_policy(TreeParams(2, 3, 13); memory_profile=:spill)
        @test spill_mem.autodiff == :charge_adjoint
        @test spill_mem.checkpointed
        @test spill_mem.checkpoint_max_ram_checkpoints == 1
        @test spill_mem.charge_cache_spill
        ...
        @test_throws ArgumentError QaoaXorsat.optimization_policy(TreeParams(2, 3, 13); memory_profile=:unknown)
    ...
    @testset "optimize_angles spill mode requires disk dir" begin
        @test_throws ArgumentError optimize_angles(
            TreeParams(2, 3, 1);
            clause_sign=-1,
            restarts=0,
            ...
            autodiff=:charge_adjoint,
            charge_cache_spill=true,
            checkpoint_disk_dir=nothing,
            initial_guesses=[QAOAAngles([0.7], [0.3])],
            rng=MersenneTwister(141),
        )
    end

    @testset "preflight blocks unsafe charge adjoint" begin
        @test_throws ArgumentError optimize_angles(
            TreeParams(2, 3, 17);
            clause_sign=-1,
            restarts=0,
            maxiters=1,
            autodiff=:charge_adjoint,
            initial_guesses=[QAOAAngles(fill(0.1, 17), fill(0.2, 17))],
