...
#   julia --project=. scripts/run_p14_d_warm.jl 4 --spill
#   julia --project=. scripts/run_p14_d_warm.jl 4 --spill --spill-dir /fast/ssd/tmp
#
# Output files:
...
function parse_args()
    if isempty(ARGS)
        error("Usage: julia --project=. scripts/run_p14_d_warm.jl <D> [--spill] [--spill-dir <path>]")
    end
    ...
    while i <= length(ARGS)
        ...
        if arg == "--spill"
            spill = true
            i += 1
        elseif arg == "--spill-dir"
            i + 1 <= length(ARGS) || error("--spill-dir requires a path")
            spill = true
            spill_dir = ARGS[i + 1]
            i += 2
        else
    ...
    spill_dir === nothing && spill && (spill_dir = joinpath(REPO_ROOT, "results", "spill-cache"))
    return (D=D, spill=spill, spill_dir=spill_dir)
end
...
function main()
    ...
    println("Spill mode: ", spill ? "enabled" : "disabled")
    spill && println("Spill dir:  ", spill_dir)
    ...
    t_opt = @elapsed result = optimize_angles(
        ...
        charge_cache_spill = spill,
        checkpoint_disk_dir = spill ? spill_dir : nothing,
