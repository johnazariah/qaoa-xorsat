...
> **Impact**: Reduces exact finite-D constraint fold from O(4^{kp}) to O(p^2 · 4^p)
> **Source branch**: `feature/wht-research`
...
2. ĝ = WHT(g)                                       O(N log N)
3. Ŵ = ĝ^2  (element-wise square)                   O(N)
...
Total: $O(N \log N) = O(p \cdot 4^p)$ per fold step. Over p steps:
$O(p^2 \cdot 4^p)$.
...
The $(k-1)$-fold auto-convolution has WHT-domain representation
$\hat{g}^{k-1}$. The full computation:
...
| Max exact p ≈ 5-7 | Max exact p ≈ 15-17 |
| Need D→∞ approximation for p > 7 | Exact at any D up to memory limit |
| Open Question 1 in P1.3 spec | Resolved |

## Novelty

The Basso 2021 paper quotes $O(p \cdot 4^{pq})$ for the finite-D iteration at
constraint arity q. The Villalonga reference code implements only the D→∞
iteration. No published work currently tracked in this repo uses WHT to
accelerate the finite-D constraint fold. This factorisation appears novel and
should be treated as research-grade until it has been integrated and revalidated
on the production branch.

## What Remains

- integrate the WHT fold into the P1.3 production contraction code
- validate the full fold against the brute-force oracle
- validate MaxCut results against Farhi 2025 Table 1
- write a short algebraic proof via the convolution theorem for the paper trail
