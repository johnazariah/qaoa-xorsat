...
### 9. CUDA Backend + Streaming Gradient Checkpointing (1–2 May 2026)

The GPU pipeline was Metal-only (Apple Silicon, ComplexF32).  We added
CUDA support (NVIDIA, ComplexF64) via the existing KernelAbstractions.jl
abstraction — the source GPU kernels required zero changes.  All 7 GPU
test files were refactored to auto-detect the backend via a shared
`test/gpu_test_utils.jl` utility.  A world-age bug in `gpu_backend.jl`
was fixed (Julia 1.12's stricter binding age enforcement).

Benchmarks on RTX 4060 Laptop (8 GB VRAM, compute 8.9):
- Forward eval scales cleanly from p=1 (1.6ms) through p=12 (8.2s, F64)
  and p=13 (57s, F32)
- ~2–3× GPU speedup over 16-thread CPU at p=9–10

The existing `gpu_checkpointed_forward_backward` OOM'd at p=13 because
`recompute_segment` materialises O(segment_len) GPU arrays simultaneously.
We added `gpu_streaming_forward_backward` with two key innovations:

1. **CPU-offloaded checkpoints**: every forward level's branch tensor is
   stored in host RAM via `Array(gpu_tensor)`, not GPU VRAM.  At p=13 F32
   this uses 14 GB CPU RAM but zero VRAM for checkpoints.

2. **Allocation-free streaming backward**: root backward temporaries are
   reused as working buffers (buf_Bt, buf_child, buf_fbar).  Kernel objects
   are hoisted outside the loop.  Zero GPU allocations inside the backward
   loop — the working set is a fixed ~9 vectors.

Results on RTX 4060 8GB:
- p=12 F64 gradient: 314s ✓ (was marginal with segment approach)
- p=13 F32 gradient: 1220s ✓ (previously OOM — first ever on 8GB)
- p=13 F64 gradient: OOM (fundamental — 9 × 2 GB > 8 GB)
...
Added full CUDA GPU support and a streaming gradient checkpointing
algorithm for memory-limited discrete GPUs.

#### CUDA Backend (1 May)

- CUDA.jl added to Project.toml; runtime auto-detection in
  `gpu_backend.jl` picks CUDA > Metal > CPU
- Fixed world-age bug: `getfield(Main.CUDA, :functional)` fails when the
  binding is newer than the function's world age; replaced with
  `Base.invokelatest(() -> Main.CUDA.functional())`
- All 7 GPU test files refactored: `test/gpu_test_utils.jl` provides
  GPU_OK, GPU_CT, GPU_RT, GPU_ARRAY_TYPE, gpu_array() — backend-agnostic
- 197/197 GPU tests pass on CUDA (RTX 4060, CUDA 12.9)

#### Streaming Backward (2 May)

- New `gpu_streaming_forward_backward()` in `src/gpu_checkpointed.jl`
- Checkpoints every level to CPU RAM (not GPU VRAM, not disk)
- Backward loop processes one step at a time: load B[t] from CPU,
  recompute child_hat and folded, run one backward step, repeat
- Pre-allocated GPU buffers reused from root backward temporaries
- Kernel objects hoisted outside the loop
- Verified bit-exact against `gpu_forward_backward` at p=1,2,3,5,7 for
  both MaxCut (k=2,D=3) and XORSAT (k=3,D=4)

#### Benchmarks (RTX 4060 Laptop, 8 GB VRAM)

| p  | precision | forward eval | streaming gradient | notes |
|----|-----------|-------------|-------------------|-------|
| 12 | F64       | 8.2s        | 314s              | ✓     |
| 13 | F32       | 57s         | 1220s             | ✓ NEW |
| 13 | F64       | OOM         | OOM               | needs 16GB |
| 14 | F32       | OOM         | OOM               | needs 24GB |

### Commits

- `0aa4efb` Add CUDA GPU backend support alongside Metal
- `60d206a` Fix world-age bug in gpu_backend.jl CUDA/Metal detection
- `aa0a15e` Add gpu_streaming_forward_backward for memory-limited GPUs
